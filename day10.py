...
adapter in your bag. (If your adapter list were 3, 9, and 6, your device's built-in adapter would be rated for
12 jolts.)
...
adapters = [int(i) for i in get_puzzle_input('day10')]


def get_diffs(adapters):
    adapters = sorted(adapters)
    nodes = [0] + adapters + [max(adapters) +3]
    diff1, diff3 = 0, 0
    for i in range(len(nodes)-1):
        adapter_a, adapter_b = nodes[i], nodes[i + 1]
        diff = adapter_b - adapter_a
        if diff == 1:
            diff1 = diff1 + 1
        elif diff == 3:
            diff3 = diff3 + 1
...
def find_chain(adapters, path_count=0):
    adapters = sorted(adapters)
    terminal_value = len(adapters) -1
    num_paths = 0
    for i in range(terminal_value):
        node = adapters[i]
        if i == terminal_value:
            print("at terminal value", i)
        targets = []
        for j in range(i+1, min(i+4, terminal_value)):
            target = adapters[j]
            diff = target - node
            if diff > 3:
                break
        ...
        if len(targets) > 1:
            for j in targets:
                num_paths = num_paths + find_chain(adapters[j:])
    return num_paths
...
if __name__ == '__main__':
    diff1, diff3 = get_diffs(adapters)
    print("Multiplied diffs is", diff1*diff3)