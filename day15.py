...
def last_index(lst, item):
    if not item in lst:
        return None
    return len(lst) - list(reversed(lst)).index(item)


def run_sequence(starting_numbers, upto=2020):
    turn = 1
    positions = dict()
    numbers = []
    ...
    while turn <= upto:
        #last_pos = last_index(numbers, last_number)
        last_pos = positions.get(last_number)
        penultimate_number = last_number
        if not last_pos:
            #numbers.append(last_number)
            positions[last_number] = turn
            last_number = 0
        else:
            gap = turn - last_pos
            #numbers.append(last_number)
...
if __name__ == '__main__':
    print("The 2020th number spoken is", run_sequence(upto=2020, starting_numbers=puzzle_input))
    print("The 30000000th number spoken is", run_sequence(upto=30000000, starting_numbers=puzzle_input))
